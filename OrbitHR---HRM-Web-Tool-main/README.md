...
graph TD
    ...
    AuthMod[Authentication & Authorization Middleware\nJWT Verification]
    API[Express REST API Backend\nNode.js]
    Controllers[Controllers / Business Logic\nData Formatting & Validation]
    DB[(MongoDB Database\nMongoose ODM)]
    Files[(Local File Storage\nMulter Uploads)]
    
    Client -- HTTP JSON Requests / Axios --> API
    API -- Extract Bearer Token --> AuthMod
    AuthMod -- Valid Token & Role --> Controllers
    AuthMod -- Invalid Token --> Client
    Controllers -- Read/Write Operations --> DB
    Controllers -- Process Multipart/Form-data --> Files
    DB -- Query Results --> Controllers
